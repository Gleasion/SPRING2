...
public class DatabaseConnectController {
	...
	/*
		...
		- Oracle win32_lg_client외 OracleXE112_Win64로 오라클 클라이언트, 서버를 설치
		...
		- 데이터베이스 데이터에 접근하여 처리하는 방식
			...
			> 마이바티스
				...
				*** 우리는 Mybatis를 이용한 데이터 접근 처리로 CRUD게시판 진행
		...
		5. 스프링 JDBC (PrepareStatement와 같은 방식이므로 메모만 하고 넘어감)
		- SQL로만 데이터베이스를 쉽게 처리할 수 있도록 도와주는 JDBCTemplate 클래스를 제공
			...
			# JDBCTemplate 클래스가 제공하는 주요 메소드
				...
				- query
				: ResultSetExtractor, RowCallbackHandler와 함께 조회 할 때 사용한다.
			...
			# 게시판 구현 설명
				...
				JDBC의 방식은 PrepareStatement를 활용한 방식과 동일하므로 넘어가도록 함
		...
		즉, 구현되어있는 클래스에 테이블을 매핑하기 위한 프레임워크입니다.
			...
			# Entity
				...
				- 메모리 상에 자바 객체의 인스턴스 형태로 존재하며 EntityManager에 의해 데이터 베이스의 데이터와 동기화 된다.
			...
			# JPA 관련 어노테이션
				...
				Entity임을 설정하고 @Table 어노테이션 테이블명을 설정해주면 VO클래스는 데이터베이스의 생성되어있는 테이블과 연결되기 위한
				준비가 된다.
				...
				자동으로 Entity를 분석하여 함수에 따를 쿼리를 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
				
				JPA JPA 구현체인 hibernate를 이용하여 사용합니다. (입력값 검증을 위해서 추가한 hibernate임)
				그러기 위해선 관련 의존 관계를 등록 후 사용해야합니다. (관련 라이브러리는 hibernate 말고도 설정에 따라 다양할 수 있음)
				SQL문을 이용하여 데이터베이스 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어
				객체모델을 이용하여 비지니스 로직을 구성하는데에만 집중 할 수 있다.
				하지만, 프로젝트의 규모와 복잡도가 클 때 설계가 잘못되는 경우 해당 프로젝트의 품질이 떨어진다.
				그만큼 설계라인의 정확성이 필요하고, 여러방면에서 정확한 검증이 제대로 이뤄지지 않으면 적용하기가 까다롭다.
				...
				** 데이터베이스 설계 라인이 정확하게 설계되어 있는 상태에서는 JPA가 가독성이 좋다지만, 우리는 Mybatis를 활용해 CRUD를 진행!
				...
				[용어]
					...
					** ORM이란?
						- 객체에 데이터를 읽고 쓴느 방법으로 관계형 데이터베이스에 데이터를 읽고 쓰는 기술입니다.
		...
		- SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임워크
			...
			** 마이바티스를 간단하게 살펴보고 실제 설정 및 자세한 내용은 MybatisController에서 진행
			...
			# 의존 관계 정의
				...
				- pom/xml에서 mybatis 설정을 위한 의존 관계를 등록하도록 합니다.
					> mybatis
					> mybatis-spring
					> spring-jdbc
					> commons-dbcp2
